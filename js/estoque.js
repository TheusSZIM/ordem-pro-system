...
// Painel de flags manuais do Kanban
// ============================================

const ESTOQUE_FLAGS_KEY = 'kanban_flags_vazios';

/* Carrega todos os flags salvos */
function getFlags() {
    try { return JSON.parse(localStorage.getItem(ESTOQUE_FLAGS_KEY) || '{}'); }
    catch(_) { return {}; }
}

/* Salva flags */
function saveFlags(flags) {
    localStorage.setItem(ESTOQUE_FLAGS_KEY, JSON.stringify(flags));
}

/* Adiciona flag numa posição */
function marcarPosicaoVazia(posKey, modelo, operador) {
    ...
    flags[posKey] = {
        ...
        status:   'pendente',   // pendente | regularizado
    };
    saveFlags(flags);
    return flags[posKey];
}

/* Remove flag (regularizado) */
function regularizarPosicao(posKey) {
    const flags = getFlags();
    if (flags[posKey]) {
        flags[posKey].status       = 'regularizado';
        flags[posKey].dataRegularizacao = new Date().toISOString();
        saveFlags(flags);
    }
    renderEstoque();
}
...
function renderEstoque() {
    ...
    const flags    = getFlags();
    const todos    = Object.values(flags).sort((a,b) => new Date(b.dataHora)-new Date(a.dataHora));
    ...
    lista.innerHTML = todos.map(f => {
        ...
        const dt = new Date(f.dataHora);
        const dtStr = dt.toLocaleDateString('pt-BR') + ' ' + dt.toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'});
        const dtReg = f.dataRegularizacao
            ? new Date(f.dataRegularizacao).toLocaleDateString('pt-BR') + ' ' + new Date(f.dataRegularizacao).toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'})
            : null;
        ...
        <div class="flag-card ${isPendente?'flag-pendente':'flag-ok'} rounded-xl p-4 border space-y-3">
            ...
            <div class="flex items-start justify-between gap-2">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="font-bold text-slate-900 dark:text-white text-base">${f.posKey}</p>
            ...
            <div class="grid grid-cols-2 gap-2 text-xs">
                ...
                <div class="bg-emerald-50 dark:bg-emerald-900/20 rounded-lg p-2.5 col-span-2">
                    <p class="text-emerald-500 mb-0.5">Regularizado em</p>
            ...
            <div class="flex gap-2 pt-1">
                ${isPendente ? `
                <button onclick="regularizarPosicao('${f.posKey}')"
                ...
                <button onclick="removerFlag('${f.posKey}')"
...
document.addEventListener('pageChanged', e => {
    if (e.detail === 'estoque') renderEstoque();
});
