        ...
        <div class="fixed inset-0 z-[80] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" id="ordem-detail-modal">
            <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-lg flex flex-col max-h-[90vh]">
                ...
                <div class="flex-1 overflow-y-auto px-5 py-5">
                    ...
                    <!-- CAMPO LOTE — editável -->
                    <div class="flex items-center gap-3 mb-5 p-3 rounded-xl" style="background:rgba(148,163,184,.07);border:1px solid rgba(148,163,184,.12);">
                        <span class="material-symbols-rounded text-slate-400 text-lg flex-shrink-0">label</span>
                        <div class="flex-1 min-w-0">
                            <p class="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-0.5">Lote</p>
                            <div class="flex items-center gap-2" id="lote-view-mode">
                                <span class="text-sm font-semibold text-white" id="modal-lote-display">
                                    ${ordem.lote || '—'}
                                </span>
                                <button onclick="
                                    document.getElementById('lote-view-mode').classList.add('hidden');
                                    document.getElementById('lote-edit-mode').classList.remove('hidden');
                                    document.getElementById('lote-input').value='${(ordem.lote||'').replace(/'/g,'\'')}';
                                    document.getElementById('lote-input').focus();
                                " class="p-1 rounded text-slate-500 hover:text-slate-300 transition-colors" title="Editar lote">
                                    <span class="material-symbols-rounded text-sm">edit</span>
                                </button>
                            </div>
                            <div class="hidden items-center gap-2" id="lote-edit-mode">
                                <input id="lote-input" type="text" placeholder="Ex: LOTE-2024-001"
                                    ...
                                    onkeydown="if(event.key==='Enter'){window._salvarLoteOrdem('${ordem.id}',this.value);document.getElementById('lote-view-mode').classList.remove('hidden');document.getElementById('lote-edit-mode').classList.add('hidden');}if(event.key==='Escape'){document.getElementById('lote-view-mode').classList.remove('hidden');document.getElementById('lote-edit-mode').classList.add('hidden');}">
                                <button onclick="window._salvarLoteOrdem('${ordem.id}',document.getElementById('lote-input').value);document.getElementById('lote-view-mode').classList.remove('hidden');document.getElementById('lote-edit-mode').classList.add('hidden');"
                                    class="px-2 py-1 rounded-lg text-xs font-bold text-white transition-colors"
                                    style="background:#4f46e5;">Salvar</button>
                                <button onclick="document.getElementById('lote-view-mode').classList.remove('hidden');document.getElementById('lote-edit-mode').classList.add('hidden');"
                                    class="px-2 py-1 rounded-lg text-xs text-slate-400 hover:text-slate-200">✕</button>
                            </div>
                        </div>
                    </div>
